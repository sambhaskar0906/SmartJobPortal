...
import { fetchAllRecruiter, makeAnconstantId, makeAnViewId } from '../../../../reduxSlice/recruiterSlice';
...
const RecruiterList = () => {
    ...
    const filteredData = useMemo(() => {
        if (!searchQuery) return recruiter?.data || [];
        return recruiter?.data?.filter((item) => {
                ...
                <Box sx={{ p: 2 }}>
                    <Grid container alignItems={'center'}>
                        ...
                        <Grid item xs={12} md={3}>
                            <Box sx={{ width: '100%', backgroundColor: '#FFF', display: 'flex', borderRadius: '50px', px: spacing(1), alignItems: 'center', border: '1px solid #eee' }}>
                    ...
                    <Table aria-label="a dense table">
                        <TableHead>
                            <TableRow style={{ backgroundColor: palette.primary.main }} >
                                {['S.No', 'Image', 'Name', 'Email Id', 'Actions', 'Mobile No', 'Job Function', 'Address'].map((header) => (
                                    <TableCell
                                        key={header}
                            ...
                            {loading ? (
                                ...
                                paginatedData.map((item, index) => (
                                    <TableRow sx={tableCellHoverStyles} key={item._id}>
                                        <TableCell sx={{ fontSize: '11px' }} align="center">{page * rowsPerPage + index + 1}</TableCell>
                                        ...
                                        <TableCell sx={{ fontSize: "11px" }} align="left">{`${item?.name?.first_name} ${item?.name?.last_name}`}</TableCell>
                                        <TableCell sx={{ fontSize: '11px' }} align="left">{item.email}</TableCell>
                                        <TableCell sx={{ fontSize: '11px' }} align="center">
                                        ...
                                        <TableCell sx={{ fontSize: '11px' }} align="left">{item.mobile}</TableCell>
                                        <TableCell sx={{ fontSize: '11px' }} align="left">{item.job_function}</TableCell>
                                        <TableCell sx={{ fontSize: '11px' }} align="left">{item?.current_location?.locality}, {item?.current_location?.pin_code},{item?.current_location?.city},{item?.current_location?.state}</TableCell>
                                    </TableRow>
                                ))
                            ) : (
                                <TableRow>
